...
:: Contient des références %DEP% et %FIC% :
...
"${model.postgresqlBinPath}"\psql -v ON_ERROR_STOP=1 -p ${model.modelDb.port} -d ${model.modelDb.name}  -h ${model.modelDb.hostname} -U ${model.modelDb.user} -c "\copy %TABLE% FROM '%CSV_FILE%' delimiter '|' csv"
